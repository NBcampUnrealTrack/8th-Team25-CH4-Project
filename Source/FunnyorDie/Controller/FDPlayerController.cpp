...
void AFDPlayerController::SetupInputComponent()
{
	...
	/*
	색상 메뉴 토글 입력 바인딩.
	UEnhancedInputComponent는 레거시 BindKey 오버로드를 = delete로 막아놨기 때문에
	(Enhanced Input 프로젝트에서 두 입력 체계를 섞지 못하게 하려는 의도)
	다른 입력들과 동일하게 InputAction으로 바인딩해야 한다.
	*/
	if (IA_ColorMenu)
	{
		EIC->BindAction(IA_ColorMenu, ETriggerEvent::Started, this, &AFDPlayerController::Input_ColorMenu);
	}
...
void AFDPlayerController::Input_ColorMenu(const FInputActionValue& Value)
{
	// 이미 떠 있으면 닫고, 없으면 연다
	const bool bAlreadyOpen = ColorMenuWidgetInstance && ColorMenuWidgetInstance->IsInViewport();
	ToggleColorMenu(!bAlreadyOpen);
}

void AFDPlayerController::ToggleColorMenu(bool bOpen)
{
	if (bOpen)
	{
		if (!ColorMenuWidgetClass)
		{
			UE_LOG(LogTemp, Warning, TEXT("[플레이어 컨트롤러] ColorMenuWidgetClass가 할당되지 않음"));
			return;
		}

		if (!ColorMenuWidgetInstance)
		{
			ColorMenuWidgetInstance = CreateWidget<UFDColorMenuWidget>(this, ColorMenuWidgetClass);
		}
		if (!ColorMenuWidgetInstance) return;

		// 내 캐릭터의 커스터마이징 컴포넌트를 넘겨줘야 버튼이 서버 요청을 보낼 수 있음
		if (ACharacter* MyCharacter = Cast<ACharacter>(GetPawn()))
		{
			if (UFDCustomizationComponent* CustomComp =
				MyCharacter->FindComponentByClass<UFDCustomizationComponent>())
			{
				ColorMenuWidgetInstance->InitializeMenu(CustomComp);
			}
		}

		if (!ColorMenuWidgetInstance->IsInViewport())
		{
			ColorMenuWidgetInstance->AddToViewport();
		}

		/*
		이모트 메뉴와 같은 이유로 입력 모드 전환을 한 틱 미룬다.
		지금 프레임에서 바로 SetInputMode를 부르면 뷰포트 포커스가 바뀌면서
		방금 처리 중인 키 입력 상태가 리셋되어 눌림과 뗌 판정이 꼬인다.
		GameAndUI로 여는 이유는 색을 바꾸는 동안 뒤에 내 캐릭터가 보여야 하기 때문이다.
		*/
	...
	{
		if (ColorMenuWidgetInstance && ColorMenuWidgetInstance->IsInViewport())
		{
			ColorMenuWidgetInstance->RemoveFromParent();
		}
	...
	UE_LOG(LogTemp, Log, TEXT("[플레이어 컨트롤러] 색상 메뉴 %s"), bOpen ? TEXT("열림") : TEXT("닫힘"));
}
